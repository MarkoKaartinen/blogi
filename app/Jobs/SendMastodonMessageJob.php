...
class SendMastodonMessageJob implements ShouldQueue
{
    ...
    public function __construct(public Mastodonable $mastodonable) {}

    public function handle(): void
    {
        $url = config("services.mastodon.instance") . "/api/v1/statuses";
        $response = Http::withToken(config("services.mastodon.token"))->post(
            $url,
            [
    ...
    public function middleware(): array
    {
        return [
            new WithoutOverlapping(
                get_class($this->mastodonable) .
                    ":" .
                    $this->mastodonable->getKey(),
            ),
        ];
    }
}
