...
<div>
    ...
    <div x-data="{ showCommentForm: false }" @hidecommentform="showCommentForm = false">
        <div class="flex items-center flex-wrap gap-6">
            @if($mastodonPostId)
                <a class="text-lg inline-flex items-center border-2 rounded-xl border-nord-10 px-3 py-3 bg-nord-10 transition-colors duration-300 hover:bg-nord-1" href="{{ config('services.mastodon.profile_url') }}/{{ $mastodonPostId }}" target="_blank">
        ...
        <div x-cloak x-show="showCommentForm">
            <div class="pt-6">
                <livewire:post-comment :commentable-id="$commentableId" :commentable-type="$commentableType" />
            </div>
        </div>
    </div>

    <livewire:show-comments :mastodon-status="$mastodonPostId" :commentable-id="$commentableId" :commentable-type="$commentableType" />
</div>
